...
class App:
    ...
    def help_window(self, window, file, window_height=400, window_width=800):
        """
        ...
        try:
            help_file = open(file, 'r')
        except FileNotFoundError as e:
            message = '\nThe help document ' + file + ' cannot be found.\n'
            message+= str(e) + '\n'
            raise FileNotFoundError(message)
        help_window = self.pop_up_window(window, 'Help', window_height, window_width, resizable = True)
        self.set_up_menu(help_window)
        text = self.text_box_fill(help_window)
        text.insert(tk.INSERT, help_file.read())
        self.apply_scrollbars(help_window, text, True, True)
        help_file.close()
        return help_window

    def text_box_fill(self, window):
        """
        Creates a text box which will will a window completely.
    
        Arguments:
            (arg1) window (tk.Toplevel) : containing window
    
        Returns:
            (ret1) text (tk.Text) : text widget filling the entire window
        """
        ...
        self.apply_scrollbars(window, text, True, True)
        return text
